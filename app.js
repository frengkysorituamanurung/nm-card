...
const addCardComponent = (
  ...
  console.log(image, instagram,  url, title);
  const isExpired = getExpiration();
  const cardElement = isExpired
    ? ''
    : `
        <div class="card">
          ${
            section === 'Network Management 21'
              ? `<img
          src="${image}"
          class="card__image"
        /> <div class="card__actions">
        <a href="${instagram}" target="_blank" rel="noopener" class="card__actions__button">Instagram</a>
        <a href="" target="_blank" rel="noopener" class="card__actions__button" hidden></a>
      </div>`
              : ''
          }
          <div class="card__header">
            <a href="${url || instagram}" target="_blank"
        rel="noopener noreferrer" class="card__header__title">${title}</a>
